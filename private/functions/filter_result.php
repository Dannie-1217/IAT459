...
if(!empty($pet_type) && empty($location)){
    $general_query = "SELECT pet.pet_id, pet.pet_name, pet.location, pet.post_date, MIN(pet_images.images) AS pet_imgs
                    ...
                    WHERE pet.pet_type = '".$pet_type."' AND pet.status != 'Adopted' 
                    GROUP BY pet.pet_id
    ...
    $general_query = "SELECT pet.pet_id, pet.pet_name, pet.location, pet.post_date, MIN(pet_images.images) AS pet_imgs
                    FROM pet 
                    JOIN pet_images ON pet.pet_id = pet_images.pet_id 
                    WHERE pet.location = '".$location."' AND pet.status != 'Adopted'
                    GROUP BY pet.pet_id
                    ORDER BY pet.post_date DESC
                    LIMIT $itemPerPage OFFSET $offset;";
    ...
    $general_query = "SELECT pet.pet_id, pet.pet_name, pet.location, pet.post_date, MIN(pet_images.images) AS pet_imgs
                    FROM pet 
                    JOIN pet_images ON pet.pet_id = pet_images.pet_id 
                    WHERE pet_type = '".$pet_type."' AND pet.location = '".$location."' AND pet.status != 'Adopted'
                    GROUP BY pet.pet_id
                    ORDER BY pet.post_date DESC
                    LIMIT $itemPerPage OFFSET $offset;";
